#!/bin/sh -p
# @ author - https://www.askapache.com/
...

{
   ...
   then
	  ...
	  __T "CURRENT PROCESS CMDLINE"
	  {
		 ...
		 do
			d=${p/*\/};
			test -r /proc/${d}/cmdline || continue;
			echo -en "[${d}]\t " && sed "s/\x00/ /g" /proc/${d}/cmdline;
			echo;
		 done | sed "/]\t [\t ]*$/d";
	  }
	  __CT
   ...
   {
	  __A {a..z} {A..Z} _{0..9} _{A..Z} _{a..z} | cat -Tsv
   }
   __CT
   ...
   __T "ENV AND EXPORT"
   {
	  __H env && command env | cat -Tsv && __P '-'
	  builtin export | cat -Tsv
   }
   __CT
   ...
   then
	  __T "RUBY VARIABLES"
	  {
		 ruby -e 'ENV.each do |i|; puts i; end' | cat -Tsv
	  }
	  __CT
   fi
   ...
   then
	  __T "PERL VARIABLES"
	  {
	     perl -e 'for my $var ( sort keys %ENV ) {printf "%s = \"%s\"\n", $var, $ENV{$var}; }' | cat -Tsv
	  }
	  __CT
   fi
   ...
   then
	  __T "OCAML VARIABLES"
	  {
	     echo -e "#load \"unix.cma\";; open Unix;; let env = ( Unix.environment ()) in let len = Array.length env in for i = 1 to len - 1 do print_string env.(i); print_newline (); done;" | ocaml /dev/stdin | sort | cat -Tsv
	  }
	  __CT
   fi
   ...
   then
	  __T "PHP VARIABLES"
	  {
	     php -d log_errors=0 -d error_reporting=3424349 -d display_startup_errors=1 -d display_errors=1 -r 'var_export( array( "_ENV" => $_ENV, "_SERVER" => $_SERVER ) );'
	  }
	  __CT
   fi
   ...
   if __H php-cgi;
   then
	  __T "PHP-CGI VARIABLES"
	  {
	     #echo '<?php var_export( array( "_ENV" => $_ENV, "_SERVER" => $_SERVER ) ); ?>' |  (REDIRECT_STATUS=200 php-cgi -i )
	     unset ${!REQUES*}
	     unset ${!SCRIPT*}
	     unset ${!SERVER*}
	     unset ${!GATE*}
	     unset ${!REMOTE*}
		 echo '<?php ob_start(); phpinfo(INFO_VARIABLES); $g=ob_get_clean(); $gg=explode("</tr>", $g, 2); echo strip_tags($gg[1]); ?>' | php-cgi -q --
	  }
	  __CT
   fi
